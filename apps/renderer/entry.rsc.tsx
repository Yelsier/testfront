...
export default async function handler(request: Request): Promise<Response> {
  ...
  const rscPayload: RscPayload = {
    root: url.searchParams.has("__partial") ? (
      <App modules={data.modules} seo={data.seo} />
    ) : (
      <Root modules={data.modules} seo={data.seo} path={url.pathname} />
    ),
    formState,
    returnValue,
  };
  ...
  const isRscRequest =
    url.searchParams.has("__rsc") || accept.includes("text/x-component");
  ...
  const ssrEntryModule = await import.meta.viteRsc.loadModule<
    typeof import("./entry.ssr.ts")
  >("ssr", "index");
  let htmlStream: ReadableStream<Uint8Array<ArrayBufferLike>>;
  try {
    htmlStream = await ssrEntryModule.renderHTML(rscStream, {
      formState,
